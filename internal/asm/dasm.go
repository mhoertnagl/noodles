...
func (m *Disassembler) Disassemble(code AsmCode) []string {
...
func (m *Disassembler) DisassembleToStr(code AsmCode) string {
	var buf bytes.Buffer
	for _, line := range m.Disassemble(code) {
...
func (m *Disassembler) opMeta(op vm.Op) (*vm.OpMeta, bool) {
	meta, err := vm.LookupMeta(op)
	return meta, err == nil
}

func (m *Disassembler) opName(op vm.Op) string {
...
func (m *Disassembler) writeInstr(ins *AsmIns) {
	var buf bytes.Buffer
	buf.WriteString("  ")
